        ...
        public async Task<List<Ejemplo>> ObtenerEjemplosPorCategorias(List<string> categoriaLabels)
        {
            ...
            else
            {
                ...
                query = $@"
                    ...
                    WHERE ec.CategoriaId IN (SELECT Id FROM RecursiveCategories);"; // Filtrar por Ids en la jerarquía}
                                                                                    // Generar la cláusula WHERE para múltiples categorías
            ...
            {
                throw new Exception("Error al obtener ejemplos por categorías", ex);
            }
            finally
            {
                await _conexion.CloseAsync();
